...
class topicModify extends postModify {
	...
	function getData() {
		return $this->db->get_one("SELECT t.*,tm.content,tm.aid,tm.ifsign,tm.tags,tm.magic FROM pw_threads t LEFT JOIN $this->pw_tmsgs tm USING(tid) WHERE t.tid=" . pwEscape($this->tid));
	}
	...
	function afterModify() {
		...
		/*
		$rt = $this->db->get_one("SELECT lastpost FROM pw_forumdata WHERE fid=" . pwEscape($this->forum->fid));
		$lastpost = explode("\t", $rt['lastpost']);

		if ($lastpost[2] == $this->atcdb['postdate']) {
			$this->forum->lastinfo('modify');
		}
		*/
	}
}
