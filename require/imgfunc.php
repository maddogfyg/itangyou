...
function ImgWaterMark($source,$w_pos=0,$w_img='',$w_text='',$w_font=12,$w_color='#FF0000',$w_pct,$w_quality,$dstsrc=null){
	...
	if ($w_pos == 0) {
		...
		$wY = $ifWaterImage ? rand(0,($sourcedb['height'] - $waterdb['height'])) : rand($waterdb['height'],$sourcedb['height']);
	} elseif ($w_pos == 1) {
		$wX = 5;
		$wY = $ifWaterImage ? 5 : $waterdb['height'];
	} elseif ($w_pos == 2) {
		...
		$wX = 5;
		$wY = $ifWaterImage ? $sourcedb['height'] - $waterdb['height']-5 : $sourcedb['height']-5;
	} elseif ($w_pos == 5) {
		$wX = ($sourcedb['width'] - $waterdb['width']) / 2;
		$wY = $ifWaterImage ? $sourcedb['height'] - $waterdb['height']-5 : $sourcedb['height']-5;
	} elseif ($w_pos == 6) {
		...
		$wX = ($sourcedb['width'] - $waterdb['width'])/2;
		$wY = $ifWaterImage ? ($sourcedb['height'] - $waterdb['height'])/2 : ($sourcedb['height'] + $waterdb['height'])/2;
	}
