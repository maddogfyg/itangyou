...
function()
{
	...
	{
	  items:[],
	  module:"{name}<f>f(){if({this}=='-'){return'<div  class=\"menu-sep-start "+(IE6?"fixedPadding":"")+"\">&nbsp;</div>'}else{return'<li url=\"{url}\" id=\"{id}-shortcut\" style=\"width:237px;\" '+({this}.disabled?'  style=color:#ccc; onfocus=\"return false;\" onmousedown=\"this.onclick=null;this.onmousedown=null;this.onmouseup=null;event.cancelBubble=true;return false;\" disabled=true ':'')+'  class=\"start_li "+(IE6?"fixedPadding":"")+"\"><a href=\"#\" '+({this}.disabled?' style=color:#ccc; onmousedown=\"return false;\" disabled':'')+'  class=\"menu_item\">{name}</a></li>'}}</f>"
	});
    ...
    PW.StartMenu.prototype.render = function()
    {
        ...
        $('startMenu').onclick = function()
        {
            ...
            p.render($('startMenu')).setHTML(upDown[_this.direct][0]+"<div class='"+(IE6?"ie6pos":"startPanel")+" menu_item'><ul class='menu_li' >"+desk.list.simple(_this.items, _this.module, '')+"</ul></div>"+(IE6?"<div  style=position:relative;left:0;top:"+topFix+";width:100%;z-index:-1 class=startPanel><img src=js/desktop/css/windoo/s.gif onload=\"this.parentNode.style.height=this.parentNode.previousSibling.offsetHeight;this.parentNode.nextSibling.style.marginTop=this.parentNode.style.top=-this.parentNode.previousSibling.offsetHeight\"></div>":"")+upDown[_this.direct][1]).onclick(function()
            {
