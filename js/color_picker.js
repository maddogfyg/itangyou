...
function showColorPicker(inputObj,formField,reback){
	...
	color_picker_div.style.left = findPosX(inputObj) + (getObj('upPanel') ? 0 : getLeft()) + (is_ie ? 4:0) + 'px';
	color_picker_div.style.top = findPosY(inputObj) + (getObj('upPanel') ? 0 : getTop()) + inputObj.offsetHeight + (is_ie ? 4:0) + 'px';
