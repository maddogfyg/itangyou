...
if(empty($action)){
	...
	if($updateHot=="updateHot"){
		...
		InitGP(array_merge(array("id","currentFilterType","currentFilterTime","active","display",
		...
		$sqlUpdateHot = "UPDATE pw_modehot SET type_name=".pwEscape($typeName).",
						filter_type=".pwEscape(serialize($filterTypeData)).", 
						filter_time=".pwEscape(serialize($filterTimeData)).", 
						display=".pwEscape($display).",
						active=".pwEscape($active)." WHERE id=".pwEscape($id);
