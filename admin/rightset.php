			...
			foreach ($left as $title => $value) {
				...
				foreach ($value['option'] as $key => $option) {
					if (isset($option[0]) && !is_array($option[0])) {
						...
						$editset .= ' <li><input type="checkbox" name="rightdb['.$key.']" value="1" '.$checked.'> '.$option[0].'</li>';
					} else {
						foreach ($option as $k => $v) {
							$checked = (!empty($right) && (int)$right[$k]==1) ? 'CHECKED' : '';
							$editset .= ' <li><input type="checkbox" name="rightdb['.$k.']" value="1" '.$checked.'> '.$v[0].'</li>';
						}
					}
				}
				$editset .= "</ul></td></tr>";
