...
if (empty($action)) {
	...
	$query = $db->query("SELECT b.*, m.username FROM pw_banuser b LEFT JOIN pw_members m ON b.uid=m.uid $sql ORDER BY id DESC $limit");
	...
	$uids1 = $uids2 = array();
	$query = $db->query("SELECT * FROM pw_banuser WHERE id IN ($ids)");
	while ($rt = $db->fetch_array($query)) {
		if ($rt['fid']) {
			$uids2[] = $rt['uid'];
		} else {
	...
	$db->update("DELETE FROM pw_banuser WHERE id IN($ids)");
	$uids1 && $db->update("UPDATE pw_members SET groupid='-1' WHERE uid IN(".pwImplode($uids1).")");
	$uids2 && $db->update("UPDATE pw_members m LEFT JOIN pw_banuser b ON m.uid=b.uid AND b.fid>0 SET m.userstatus=m.userstatus&(~1) WHERE b.uid is NULL AND m.uid IN(".pwImplode($uids2).")");
