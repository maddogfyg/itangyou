...
function updatecache_newinfo() {
	global $db;
	$nf_config = $nf_newinfodb = $nf_order = array();
	$query = $db->query("SELECT db_name,db_value FROM pw_config WHERE db_name LIKE 'nf\_%'");
	while ($rt = $db->fetch_array($query)) {
		$rt['db_name'] = preg_replace('/[^\d\w\_]/is','',$rt['db_name']);
		${$rt['db_name']} = (array)unserialize($rt['db_value']);
	...
	$newinfodb = "<?php\r\n";
	$newinfodb .= '$nf_config='.pw_var_export($nf_config).";\r\n";
	$newinfodb .= '$nf_newinfodb='.pw_var_export($nf_newinfodb).";\r\n";
	$newinfodb .= '$nf_order='.pw_var_export($nf_order).";\r\n";
	$newinfodb .= "\r\n?>";
