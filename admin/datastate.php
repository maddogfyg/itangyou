...
if (empty($action)) {
	...
	switch ($type) {
		...
		case 'reply':
			...
			if ($db_plist) {
				foreach (explode(',',$db_plist) as $key=>$value) {
					$sql_1 .= " UNION ALL SELECT COUNT(*) AS replies,DAYOFMONTH(FROM_UNIXTIME(postdate)) AS day FROM pw_posts{$value} WHERE postdate BETWEEN ".pwEscape($timestart).' AND '.pwEscape($timeend).'GROUP BY day';
				}
			}
		...
		case 'postmen':
			...
			if ($db_plist) {
				foreach (explode(',',$db_plist) as $key=>$value) {
					$sql_2 .= " UNION SELECT DAYOFMONTH(FROM_UNIXTIME(postdate)) AS day,authorid FROM pw_posts{$value} WHERE postdate BETWEEN ".pwEscape($timestart).' AND '.pwEscape($timeend)."GROUP BY day,authorid";
				}
			}
	...
	switch ($type) {
		...
		case 'reply':
			...
			if ($db_plist) {
				foreach (explode(',',$db_plist) as $key=>$value) {
					$sql_1 .= " UNION ALL SELECT COUNT(*) AS replies FROM pw_posts{$value} WHERE postdate BETWEEN ".pwEscape($timestart).' AND '.pwEscape($timeend);
				}
			}
			$query = $db->query($sql_1);
			while ($rt = $db->fetch_array($query)) {
		...
		case 'postmen':
			...
			if ($db_plist) {
				foreach (explode(',',$db_plist) as $key=>$value) {
					$sql_2 .= " UNION SELECT authorid FROM pw_posts{$value} WHERE postdate BETWEEN ".pwEscape($timestart).' AND '.pwEscape($timeend)." GROUP BY authorid";
				}
			}
			if ($db->server_info() > '4.1') {
