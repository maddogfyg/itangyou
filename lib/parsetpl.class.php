...
class PW_ParseTpl{
	...
	function PW_ParseTpl(){

	}
	function init($tplid,$name,$ifloop=0){
		$this->index	= 1;
	...
	function getTagCode(){
		...
		if ($this->tplid) {
			$pwtpl	= L::loadDB('Tpl');
			$tpldata = $pwtpl->getData($this->tplid);
			$tagcode = $tpldata['tagcode'];
		}
		return $tagcode;
	}

	function parseTagCode(){
		$string = $this->getTagCode();
