...
function showDefaultMagic(){
	...
	if(F){
		...
		var sT = "";
		for(i in mDef){
			sT += '<img class="lhcl_slideImage" style="margin:2px" id="'+i+'" src="'+myshow+i+'J.jpg" title="'+mDef[i]+'" onclick="selectMagic(\''+i+'\');" ondblclick="setMagic(this.id)">';
		}
...
function initMagic(){
	...
	var num = 0;
	var b='<span style="float:right;margin-right:2px;width:auto;cursor:pointer" onclick="clearp(\'magiclist\');closep();showDefaultMagic();" title="close"><img src='+imgpath+'/close.gif></span>';
	for(f in magicid){
		b += '<div id="mb_'+num+'" style="float:left" onclick="showMagics(\'mb_'+num+'\','+magicid[f]+',1);">'+magicname[f]+'</div>';
		num++;
	}
