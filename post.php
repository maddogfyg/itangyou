...
InitGP(array('action','special','article','pid','page','cateid','modelid','pcid'));
$replacedb = array();
$special   = (int)$special;
$cateid	   = (int)$cateid;
$modelid   = (int)$modelid;
...
if ($action == 'new' && $pwforum->foruminfo['forumcate'] == 1) {
	if (!($pwforum->foruminfo['allowtype'] & pow(2,$special))) {
	...
	$cateid = $modelid = $pcid = 0;
} elseif ($action == 'new' && $pwforum->foruminfo['forumcate'] == 2) {
	!$cateid && $cateid = $pwforum->foruminfo['cateid'];
	if (!$modelid) {
		list($modelid) = explode(',',$pwforum->foruminfo['modelid']);
	}
	if ($cateid && $modelid) {//判断分类主题是否合法
		@include_once(D_P.'data/bbscache/topic_config.php');

		$pwforum->foruminfo['cateid'] != $cateid && Showmsg('forum_topic_unfined');
		if (strpos(",".$pwforum->foruminfo['modelid'].",",",".$modelid.",") === false) {
			Showmsg('forum_model_unfined');
		}
		!$topiccatedb[$cateid]['ifable'] && Showmsg('topic_cate_unable');
		!$topicmodeldb[$modelid]['ifable'] && Showmsg('topic_model_unable');
	}
	$special = $pcid = 0;
} elseif ($action == 'new' && $pwforum->foruminfo['forumcate'] == 3) {
	if (!$pcid) {
		list($pcid) = explode(',',$pwforum->foruminfo['pcid']);
	}
	if ($pcid) {
		...
		!$postcatedb[$pcid]['ifable'] && Showmsg('forum_pc_unfined');
	}
	$special = $cateid = $modelid = 0;
}
...
if (empty($_POST['step'])) {
	...
	*/
	...
	if ($_POST['_hexie'] != GetVerify($onlineip.$winddb['regdate'].$fid.$tid)) {
