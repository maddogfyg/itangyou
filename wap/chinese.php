...
define('_EncodePath',R_P.'wap/encode/');
...
class Chinese {
	...
	function initConvert($SourceLang,$TargetLang,$ForceTable) {
		if (($SourceLang = $this->getCharset($SourceLang)) && ($TargetLang = $this->getCharset($TargetLang)) && $SourceLang != $TargetLang) {
	...
	function CHStoUTF8($srcText) {
		$this->getTableIndex();$tarText = '';
	...
	function CHStoUNICODE($srcText,$SourceLang='') {
		$tarText = '';
		if ($this->IconvEnabled && $SourceLang) {
			for ($i=0;$i<strlen($srcText);$i+=2) {
				if(ord($srcText[$i]) > 127) {
					$tarText .= "&#x".dechex($this->CHSUTF8toU(iconv($SourceLang,"UTF-8",$srcText[$i].$srcText[$i+1]))).";";
				} else {
					$tarText .= $srcText[$i--];
				}
			}
	...
	function Convert($srcText,$SourceLang='',$TargetLang='',$ForceTable=false) {
		...
		switch ($this->EncodeLang) {
			case 'GBKtoUTF8':
				return $this->IconvEnabled ? iconv('GBK','UTF-8',$srcText) : $this->CHStoUTF8($srcText);break;
			case 'BIG5toUTF8':
				return $this->IconvEnabled ? iconv('BIG5','UTF-8',$srcText) : $this->CHStoUTF8($srcText);break;
			case 'UTF8toGBK':
				return $this->IconvEnabled ? iconv('UTF-8','GBK',$srcText) : $this->UTF8toCHS($srcText);break;
			case 'UTF8toBIG5':
				return $this->UTF8toCHS($srcText);break;
			case 'GBKtoUNICODE':
				return $this->CHStoUNICODE($srcText,'GBK');break;
			case 'BIG5toUNICODE' :
				return $this->CHStoUNICODE($srcText,'BIG5');break;
