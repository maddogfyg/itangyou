...
function showEidt(dtid,u){
	...
	var text= elementBind('input','','input');
	text.setAttribute('type','text');
	text.value	= a[i-1].innerHTML;
	li.appendChild(text);
	text.focus();

	var ok	= elementBind('input','','bt3','padding:.2em 0;margin:0 0 0 .3em');
	ok.setAttribute('type','button');
	ok.value= '确定';

	var cancel	= elementBind('input','','bt','padding:.2em 0;margin:0 0 0 .3em');
...
function creatDiaryTypeInput(id,u){/*创建日志分类*/
	...
	var text= elementBind('input','','input','width:90px;height:17px;');//生成<input class="input" type="text">
	text.setAttribute('type','text');
	li.appendChild(text);//生成<li id='dt_add'><input class="txa dtwidth" type="text"></li>
	text.focus();

	var ok	= elementBind('input','','bt3','padding:2px;margin:0 0 0 5px');//生成<input class="bt3" type="button" value="确定" onclick="">
	ok.setAttribute('type','button');
	ok.value= '确定';
	ok.onclick	= function () {
		var name = char_cv(text.value);
	...
	var cancel	= elementBind('input','','bt','padding:2px 2px 1px;margin:0 0 0 5px');//生成<input class="bt" type="input" value="取消" style="padding:.2em 0;margin:0 0 0 .3em" onclick="">
	cancel.setAttribute('type','button');
	cancel.value= '取消';
	cancel.onclick	= function () {
		text.value = '';
