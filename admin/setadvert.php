...
if (!$action) {
	...
	while ($rt = $db->fetch_array($query)) {
		...
		$cates[$rt['ckey']] = $rt;
		$optCates .= "<option value=\"{$rt['ckey']}\">{$rt['name']}</option>";
	...
	if (empty($job)) {
		...
		$pages = numofpage($count,$page,ceil($count/$db_perpage), "$basename&ckey=$ckey&keyword=".rawurlencode($keyword).'&');
		...
		} elseif ($config['type'] == 'txt' && (!$config['title'] || !$config['link'])) {
			$basename = "javascript:history.go(-1);";
		...
		$config['width'] && $config['width'] = intval($config['width']);
		$config['height'] && $config['height'] = intval($config['height']);
