...
if ($a == 'friend') {
	...
	$article = array();
	$special = $pcid + 20;
	$tablename = 'pw_pcvalue'.$pcid;

	if ($see == 'mypost') {
		$query = $db->query("SELECT t.tid,t.authorid,t.author,t.subject,pv.begintime,pv.endtime,pv.limitnum,SUM(pm.nums) as nums FROM pw_threads t LEFT JOIN $tablename pv ON t.tid=pv.tid LEFT JOIN pw_pcmember pm ON t.tid=pm.tid WHERE t.authorid=".pwEscape($winduid)." AND t.special=".pwEscape($special)." AND t.fid!=0 GROUP BY t.tid ORDER BY t.postdate DESC");
		while ($rt = $db->fetch_array($query)) {
			$article[] = $rt;
		}
		
	} elseif ($see == 'myjoin') {
		$query = $db->query("SELECT t.tid,t.authorid,t.author,t.subject,pv.endtime,pv.payway,pm.nums,pm.ifpay,pm.pcmid FROM pw_pcmember pm LEFT JOIN $tablename pv ON pm.tid=pv.tid LEFT JOIN pw_threads t ON pm.tid=t.tid WHERE pm.uid=".pwEscape($winduid)." AND t.special=".pwEscape($special)." ORDER BY t.postdate DESC");
		while ($rt = $db->fetch_array($query)) {
			$article[] = $rt;
		}
	}
	...
	if ($see == 'post') {
		...
		if ($p_list) {
			if ($space == 1 && defined('F_M')) {
				$p_table = "<li id=\"up_post0\"><a href=\"{$nurl}ptable=0\">".getLangInfo('other','posttable')."</a></li>";
				foreach ($p_list as $key => $val) {
					$p_table .= "<li id=\"up_post$val\"><a href=\"{$nurl}ptable=$val\">".getLangInfo('other','posttable')."$val</a></li>";
				}
			} else {
				$p_table = "<li><a id=\"up_post0\" href=\"{$nurl}ptable=0\">".getLangInfo('other','posttable')."</a></li>";
				foreach ($p_list as $key => $val) {
					$p_table .= "<li><a id=\"up_post$val\" href=\"{$nurl}ptable=$val\">".getLangInfo('other','posttable')."$val</a></li>";
				}
			}
...
if ($where) {
	...
	if ($count) {
		...
		$rs = $db->query("SELECT tid,fid,author,authorid,subject,postdate,lastpost,replies,hits,titlefont,anonymous,cateid,special FROM pw_threads $force WHERE $where ORDER BY $ordertype DESC $limit");
		while ($rt = $db->fetch_array($rs)) {
			$rt['subject'] = substrs($rt['subject'],45);
			$rt['forum'] = strip_tags($forum[$rt['fid']]['name']);
			$rt['postdate'] = get_date($rt['postdate'],'Y-m-d');
			$rt['pcid'] = $rt['special'] - 20;
