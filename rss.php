...
if ($tid) {
	...
	$query = $db->query("SELECT pid,subject,author,postdate,anonymous,content FROM $pw_posts WHERE tid=".pwEscape($tid)." ORDER BY postdate DESC LIMIT $Rss_listnum");
	while ($rt = $db->fetch_array($query)) {
		$rt['content'] = substrs(stripWindCode($rt['content']),300);
		$rt['anonymous'] && $rt['author'] = $db_anonymousname;
		if ($db_htmifopen) {
			$link = "$db_bbsurl/read{$db_dir}tid-{$rt['tid']}$db_ext#$rt[pid]";
		} else {
			$link = "$db_bbsurl/read.php?tid={$rt['tid']}#$rt[pid]";
		}
		$item = array(
			'title'       =>  $rt['subject'],
